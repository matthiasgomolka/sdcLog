...
descriptive statistics for compliance with the rules and regulations set by the
RDC.
...
various types of models. It checks the calculated model and the underlying data
for compliance with the rules and regulations of the RDC.

- `sdc_extreme()`: This function is used for the automatic calculation (if
possible) of extreme values according to the rules of the research data centre.
It uses the available data and calculates the values for desired variables and
groupings in compliance with the rules. The values are calculated as averages of
a sufficient number of observations. This helps the researcher to easily follow
...
### Problematic cases
Now we turn to the problematic cases. We are checking the models `model_2` and `model_3` as well as `model_5`:
```{r model_prob1}
...
Even though this seems trivial, creating logs for your scripts is essential since it bundles all information needed by the RDC for output control.
